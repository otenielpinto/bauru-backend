...
async function importarProdutoTinyDiario() {
  ...
  await produtoEstruturaFilaRepository.insertMany(produtoUpdates);
}
...
async function modificarStatusEstoque(tenant) {
  const estoqueRepository = new EstoqueRepository(tenant.id_tenant);
  const estoqueTiny = new ProdutoTinyRepository(tenant.id_tenant);
...
async function processarEstoqueByTenant(tenant) {
  ...
  const prodTinyRepository = new ProdutoTinyRepository(id_tenant);
  const estoqueRepository = new EstoqueRepository(id_tenant);
  const tiny = new Tiny({ token: tenant.token });
  tiny.setTimeout(1000 * 10);
